...
\begin{verbatim}
\newcommand{\foo}[1]{
    \ifplastex\else\hline\hline\hline\fi
    \textbf{\Large{#1}}
    \ifplastex\else\hline\hline\hline\fi
}
...
\lineii{idref}{expands all macros, converts the result to a string, 
    ...
    This type of argument is used on commands like \macro{ref} that must
...
\lineii{newdif}{corresponds to \TeX's \macro{def} command.}
