...
document object.  The value stored under each key is a callable object.
...
key is found, the value at that key (which must be a callable object) is 
...
Again, these attributes simply need to reference any Python callable
object that returns a unicode object of the rendered output.  The 
...
done by the \class{Renderable} class.  This is a mixin class that is
...
Of course, you aren't limited to using just Python methods.  Any callable
object that accepts a node as an argument can be used.  The 
...
The DVI file is then converted into images using either \program{dvipng}
or \program{dvi2bitmap} depending on which imager is chosen (see section
\ref{sec:config-images} on how to select different imagers).
...
As in all \class{Renderer}-based renderers, each key in the ZPT renderer
returns a callable object.  In this case, each of these objects is a
callable ZPT instance\footnote{ZPT instances, by default, are not callable.
The \method{__call__} is added to the object.  This new method populates
...
The first is HTML.  HTML templates must have an extension of \file{.htm},
 \file{.html}, or \file{.zpt}.  These templates are compiled using 
...
like this, you can use the \file{.zpts} extension.  Files with this
...
\lineii{name}{the name or names of the template that is to follow. 
    ...
    spaces.}
\lineii{type}{the type of the template: xml or html.  XML templates
...
environment variable would be \environment{XHTMLTEMPLATES}.  For the ZPT
renderer, the environment variable would be \environment{ZPTTEMPLATES}.
...
simply load the ZPT renderer and set the \environment{ZPTTEMPLATES} environment
