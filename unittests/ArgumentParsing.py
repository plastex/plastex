...
class ArgumentParsing(TestCase):
    ...
    def testArgumentString(self):
        ...
        expected = [CompiledArgument('arg1', {})]
    ...
    def testArgumentString2(self):
        ...
        expected = [CompiledArgument('modifier', {'spec':'*'}),
                    CompiledArgument('opt', {'spec':'[]'}), 
                    CompiledArgument('arg1', {})]
        assert arg == expected, '"%s" != "%s"' % (arg, expected)
    ...
    def testArgumentString3(self):
        class foobar(Macro):
            args = '[ %opt ] @arg1'
        arg = foobar().compileArgumentString() 
        expected = [CompiledArgument('opt', {'spec':'[]','type':dict}), 
                    CompiledArgument('arg1', {'type':list})]
        assert arg == expected, '"%s" != "%s"' % (arg, expected)
    ...
    def testArgumentString4(self):
        class foobar(Macro):
            args = '[ %;opt ] @arg1'
        arg = foobar().compileArgumentString() 
        expected = [CompiledArgument('opt', {'spec':'[]','type':dict,'delim':';'}), 
                    CompiledArgument('arg1', {'type':list})]
        assert arg == expected, '"%s" != "%s"' % (arg, expected)
