...
class setcounter(Command):
    ...
    def invoke(self, tex):
        a = self.parse(tex)
        tex.context.counters[a['name']].setcounter(a['value'])

class addtocounter(Command):
    args = 'name:str value:int'
    def invoke(self, tex):
        a = self.parse(tex)
        tex.context.counters[a['name']].addtocounter(a['value'])

class value(Command):
    args = 'name:str'
    def invoke(self, tex):
        a = self.parse(tex)
        return tex.texttokens(tex.context.counters[a['name']].arabic)
...
class Roman(Command):
    ...
    def invoke(self, tex):
        a = self.parse(tex)
        roman = ""
        n, number = divmod(int(tex.context.counters['name']), 1000)
        roman = "M"*n
        if number >= 900:
            roman = roman + "CM"
            number = number - 900
...
class roman(Roman):
    ...
    def invoke(self, tex):
        a = self.parse(tex)
        return tex.texttokens(tex.context.counters[a['name']].roman)
...
class Alph(Command):
    ...
    def invoke(self, tex):
        a = self.parse(tex)
        return tex.texttokens(tex.context.counters[a['name']].Alph)
...
class fnsymbol(Command):
    ...
    def invoke(self, tex):
        a = self.parse(tex)
        return tex.texttokens(tex.context.counters[a['name']].fnsymbol)

class stepcounter(Command):
    args = 'name:str'
    def invoke(self, tex):
        a = self.parse(tex)
        return tex.context.counters[a['name']].stepcounter()
